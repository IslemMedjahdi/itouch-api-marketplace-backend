...
class ApiManagement:
    ...
    def get_all_categories() -> Tuple[Dict[str, Any], int]:
        try:
            ...
            response_object = {"data": categories_data, "status": "success"}
            ...
            if auth_token:
                ...
                for plan in plans:
                    ...
                    if plan_name in plan_names_set:
                        return {
                            "status": "fail",
                            "message": f"Duplicate plan name: {plan_name}",
    ...
    def get_single_api(api_id: int) -> Tuple[Dict[str, str], int]:
        try:
            ...
            if api:
                plans_data = []
                category_name = (
                    ApiCategory.query.filter_by(id=api.category_id).first().name
                )
                supplier_firstname = (
                    User.query.filter_by(id=api.supplier_id).first().firstname
                )
                supplier_lastname = (
                    User.query.filter_by(id=api.supplier_id).first().lastname
                )
                ...
                api_data = {
                    ...
                    "category": {"id": api.category_id, "name": category_name},
                ...
                response_object = {"status": "fail", "message": "Api not found"}
                return response_object, HTTPStatus.NOT_FOUND
        except Exception as e:
            response_object = {
                "status": "fail",
    ...
    @staticmethod
    def get_logged_in_supplier_single_api(
        request, api_id: int
    ...
    @staticmethod
    def update_api_info(
        request, api_id, data: Dict[str, str]
    ) -> Tuple[Dict[str, str], int]:
        auth_token = request.headers.get("Authorization")
        if auth_token:
            resp = User.decode_auth_token(auth_token)
            ...
            if not api:
                response_object = {"status": "fail", "message": "Api does not exist"}
                return response_object, HTTPStatus.NOT_FOUND
            ...
            if auth_token:
                ...
                for endpoint in endpoints:
                    ...
                    if method in endpoints_dict[url]:
                        ...
                        return {
                            "status": "fail",
                            "message": f'Duplicate method "{method}" for URL: {url}',
                        }, HTTPStatus.BAD_REQUEST
                ...
                if version:
                    return {
                        "status": "fail",
                        "message": "Version already existe",
                    }, HTTPStatus.BAD_REQUEST
    ...
    @staticmethod
    def get_logged_in_supplier_single_api_version(
        request, api_id, version
    ) -> Tuple[Dict[str, str], int]:
        try:
            auth_token = request.headers.get("Authorization")
            if auth_token:
                ...
                headers = ApiVersionHeader.query.filter_by(
                    api_id=api.id, api_version=version
                ).all()
                for header in headers:
                    header_data = {
                        "key": header.key,
        ...
        try:
            ...
            if auth_token:
                ...
                if api_version.status == "active":
                    return {
                        "status": "fail",
                        "message": "The version is already active.",
                    }, HTTPStatus.OK
        ...
        try:
            ...
            if auth_token:
                ...
                if api_version.status == "disabled":
                    return {
                        "status": "fail",
                        "message": "The version is already disabled.",
                    }, HTTPStatus.OK
    ...
    def create_header(request,data: Dict[str, str], api_id, version) -> Tuple[Dict[str, str], int]:
        try:
            ...
            if auth_token:
                ...
                response_object = {
                    "status": "success",
                    "message": "Successfully created a header.",
                }
                return response_object, HTTPStatus.OK
    ...
    def update_header(request,data: Dict[str, str], api_id, version, header_id) -> Tuple[Dict[str, str], int]:
        try:
            ...
            if auth_token:
                ...
                response_object = {
                    "status": "success",
                    "message": "Successfully updated the header.",
                }
                return response_object, HTTPStatus.OK
    ...
    def delete_header(request, api_id, version, header_id) -> Tuple[Dict[str, str], int]:
        try:
            ...
            if auth_token:
                ...
                response_object = {
                    "status": "success",
                    "message": "Successfully deleted the header.",
                }
                return response_object, HTTPStatus.OK
