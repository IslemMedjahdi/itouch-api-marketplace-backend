...
from app.main.utils.validators import is_email_valid as isEmailValid
...
class UserManagement:
    ...
    def get_single_user(user_id: int) -> Tuple[Dict[str, str], int]:
        try:
            ...
            if user:
                ...
                response_object = {"status": "success", "data": user_data}
                return response_object, HTTPStatus.OK
            else:
    ...
    @staticmethod
    def update_logged_in_user_info(
    ...
    @staticmethod
    def update_logged_in_user_password(
        request, data: Dict[str, str]
    ) -> Tuple[Dict[str, str], int]:
        auth_token = request.headers.get("Authorization")
        if auth_token:
            ...
            # Update password
            password_hash = flask_bcrypt.generate_password_hash(new_password).decode(
                "utf-8"
            )
